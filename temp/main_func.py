...
def get_step_down(
    current_price: float, grid: list[float], current_line_ind: int
) -> int:
    step = 0
...
def simulate_strategy(dataframe):
    ...
    grid_down = (
        []
    )  # grid_down = [29400.0, 28800.0, 28200.0, 27600.0, 27000.0, 25800.0, 24600.0, 23400.0, 22200.0, 21000.0, 19200.0, 17400.0, 15600.0, 13800.0, 12000.0]
    ...
    for index, row in dataframe.iterrows():
        ...
        if current_price <= grid_down[current_line_ind + 1]:
            step, current_line_ind = get_step_down(
                current_price, grid_down, current_line_ind
            )
        ...
        if (
            current_price >= grid_down[current_line_ind] + grid_up_step_usdt
            and btc_balance > 0
        ):
            step = int(
                (current_price - grid_down[current_line_ind]) // grid_up_step_usdt
